...
for (i = 0; i < size; i++)
cpy_arr[i] = array[i];
merge_recursion(0, size, array, cpy_arr);
free(cpy_arr);
}

/**
 * merge_recursion - split the array recursively
 * @l: left side
 * @r: right side
 * @a: original array
 * @cpy_arr: copied one
...
return;
m = (l + r) / 2;
merge_recursion(l, m, a, cpy_arr);
merge_recursion(m, r, a, cpy_arr);
